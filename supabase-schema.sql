...
-- Group predictions (who finishes 1st and 2nd in each group)
...
-- Bracket predictions (which teams advance to each knockout stage)
create table if not exists bracket_predictions (
  user_id uuid not null references auth.users(id),
  stage text not null, -- 'r32', 'r16', 'qf', 'sf', 'final', 'champion'
